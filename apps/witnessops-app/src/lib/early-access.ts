...
export const EARLY_ACCESS_DATA_NOTE = 'We record product actions using user, workspace and run IDs to understand activation and repeat use. Events contain no hostname or source evidence. Optional feedback is stored separately from your runs. Automatic retention/deletion is not implemented yet; contact us about removal. Reports are derived presentations, not source evidence. External Exposure source snapshots are unsigned: their digests identify the source bytes, not an authenticated issuer. One Server Security Check retains a Local Audit Proofpack with a signed receipt and detached ZIP signature, checked using pinned trust policy. Valid means package checks passed; it does not establish host security or source-system truth.';
