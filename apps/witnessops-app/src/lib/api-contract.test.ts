...
test("all app API route files and methods exactly match the declared authenticated contract", async () => {
  ...
  assert.deepEqual(actual.map(endpoint => endpoint.path).sort(), ["/api/assets", "/api/billing", "/api/billing/webhook", "/api/cli/authorize", "/api/cli/login", "/api/cli/poll", "/api/cli/server-check-capture", "/api/cli/server-checks", "/api/cli/session", "/api/early-access", "/api/events", "/api/feedback", "/api/help", "/api/invitations", "/api/linux-checks", "/api/members", "/api/runs", "/api/shared-report", "/api/shares", "/api/workspace", "/callback", "/cli/login", "/login", "/signup"]);
});
