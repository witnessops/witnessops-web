...
import { runDocsAssistantServerRuntime, isDocsAssistantRuntimeUnavailable } from '../../../witnessops-web/src/lib/docs-assistant/server-runtime';
import { evaluateDocsAssistantRefusalPolicy } from '../../../witnessops-web/src/lib/docs-assistant/refusal-policy';
import { hasLikelySecret } from '../../../witnessops-web/src/lib/server/secret-detection';
...
export function presentHelpAnswer(answer: DocsAssistantAnswer): HelpAnswer {
  if(isDocsAssistantRuntimeUnavailable(answer))throw new ApiError(503,'Documentation guidance is unavailable. Try again or open Documentation.');
...
export function createHelpService(options: { origin?:string; identity?:typeof authenticatedWebSession; enabled?:boolean; answer?:(question:string)=>Promise<HelpAnswer> }={}) {
 return async (request:Request)=>{const headers={'Cache-Control':'no-store','Referrer-Policy':'no-referrer'};
  try {
   ...
   if(hasLikelySecret(input.question)||evaluateDocsAssistantRefusalPolicy(input.question).blocked)throw new ApiError(400,'Ask about using WitnessOps. Do not include secrets or request operational actions.');
