...
type Access = {name:string;password_hash:string|null;id:string; snapshot:RecipientReport; digest:string; token_hash:string; expires_at:Date; version:number; state:string};
async function access(client: PoolClient, workspace:string, id:unknown) {
 const row = (await client.query<Access>(`SELECT s.id,(SELECT name FROM report_share_names n WHERE n.share_id=s.id) AS name,s.snapshot,s.digest,s.state,a.token_hash,a.expires_at,a.version,a.password_hash FROM report_shares s JOIN report_share_access a ON a.share_id=s.id WHERE s.id=$1 AND s.workspace_id=$2 FOR UPDATE OF s,a`,[requireId(id),workspace])).rows[0];
...
export class ShareAccessStore {
 ...
 async change(user:AppUser, workspace:string, id:unknown, expected:unknown, expires:unknown, rotate:boolean) {
  ...
  return transaction(this.pool,async client=>{
   ...
   return {name:row.name,id:row.id,token,expiresAt:until.toISOString(),version:v+1,digest:row.digest};
  });
 }
 async draft(user:AppUser, workspace:string, input:{id:unknown;token:unknown;email:unknown;requestId:unknown}, origin:string) {
  ...
  return transaction(this.pool,async client=>{
   ...
   const message=reportEmail(origin,input.token,recipient,row.snapshot,row.expires_at,key,Boolean(row.password_hash),row.name), digest=messageDigest(message);
 ...
 async send(user:AppUser, workspace:string, input:{id:unknown;token:unknown;digest:unknown;confirmed:unknown}, origin:string, sender:InvitationSender) {
  ...
  const message=await transaction(this.pool,async client=>{
   ...
   const payload=reportEmail(origin,input.token,draft.recipient,row.snapshot,row.expires_at,id,Boolean(row.password_hash),row.name);
