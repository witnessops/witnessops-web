...
type Preview = {
    linkBase?:string;reportingEnabled?:boolean;name?:string;
    version: number;
...
export function ReportShare({ workspaceId, runId, role }: {
    ...
    const [name, setName] = useState(''),[routingName,setRoutingName]=useState(''),[appliedRoutingName,setAppliedRoutingName]=useState('');
 ...
 {preview && <><label htmlFor="report-name">Report name</label><input id="report-name" value={name} maxLength={120} disabled={busy} onChange={event => { setName(event.target.value); setConfirmed(false); }}/><p className="quiet">A publisher-supplied name, not a verified assertion. Do not include confidential information. Changing it requires a new preview; published links stay unchanged.</p><button className="button secondary" disabled={busy || !name.trim()} onClick={() => void act(async () => { const next = await request({ action: 'preview', runId, name,...(routingName?{routingName}:{}) }); setPreview(next);setAppliedRoutingName(routingName); setName(reportTitle(next.snapshot)); setConfirmed(false); setUrl(''); setCopied(false); })}>Update recipient preview</button><p>Expires {new Date(preview.expiresAt).toLocaleString()}. Later checks will not update this revision. Downloaded copies cannot be recalled.</p>
 {preview.reportingEnabled&&<><label>Reporting name (optional)<input value={routingName} maxLength={48} disabled={busy||Boolean(url)} placeholder="Leave blank for a generated name" onChange={event=>{setRoutingName(event.target.value);setConfirmed(false);}}/></label><p>Public address: {preview.linkBase}. Names are public, remain reserved, and do not grant access. Avoid confidential names. Use Update recipient preview to apply a new name.</p></>}
 ...
 {preview.canPublish && !url ? <><label className="authorization"><input type="checkbox" checked={confirmed} disabled={busy} onChange={event => setConfirmed(event.target.checked)}/>I reviewed the included information and approve access for {preview.passwordProtected?'anyone with the link and password':'anyone with the link'}.</label><button className="button" disabled={busy || !confirmed || name !== reportTitle(preview.snapshot)||(routingName!==appliedRoutingName)} onClick={() => void act(async () => { const published=await request({ action: 'publish', id: preview.id, token: preview.token, digest: preview.digest, audience: preview.passwordProtected?'link_and_password':'anyone_with_link' }); setUrl(`${published.linkBase??preview.linkBase??location.origin+'/s'}#${preview.token}`); await refresh(); })}>Publish link</button></> : !url ? <p>Only an Owner can publish or revoke links.</p> : null}
