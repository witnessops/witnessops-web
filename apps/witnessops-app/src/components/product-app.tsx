...
function Header({ title, children, eyebrow = "WitnessOps", action, headingLevel = 1 }: { title: string; children?: ReactNode; eyebrow?: string; action?: ReactNode; headingLevel?: 1 | 2 }) {
  return <div className="page-heading"><div><p className="eyebrow">{eyebrow}</p>{headingLevel === 2 ? <h2>{title}</h2> : <h1>{title}</h1>}{children ? <div className="support">{children}</div> : null}</div>{action}</div>;
}
...
export function ProductApp() {
  ...
  if (access !== undefined) {
    ...
    content = <div className="welcome"><p className="eyebrow">WitnessOps · Checks</p><h1>Sign in to WitnessOps</h1><p>Choose External Exposure Check for a public hostname, or One Server Security Check for locally collected Linux evidence. Keep results and reports, then compare later checks.</p>{loading ? <p role="status">Loading workspace…</p> : <><SignInLinks /><p className="quiet">Choose your sign-in method on the secure WorkOS sign-in screen.</p></>}</div>;
  } else if (!workspace) {
