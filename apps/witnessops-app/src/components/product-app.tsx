...
function History({ workspace, assetId, report = false }: { workspace: Workspace; assetId?: string; report?: boolean }) {
  ...
  if (!runs.length) return <div className="empty"><h2>No {report ? "reports" : "observations"} yet.</h2><p>Observe a hostname to retain its first session snapshot.</p><Link className="button secondary" href="/assets/new">Add asset</Link></div>;
...
function AssetList({ workspace }: { workspace: Workspace }) {
  if (!workspace.assets.length) return <div className="empty"><h2>Your first asset starts here.</h2><p>Add a public hostname. Recommended checks are already selected; nothing runs until you authorize the observation.</p><Link className="button" href="/assets/new">Add asset</Link></div>;
...
function AssetPage({ workspace, asset, busy, onRun }: { workspace: Workspace; asset: Asset; busy: boolean; onRun: (asset: Asset) => Promise<void> }) {
  const latest = orderedRuns(workspace, asset.id)[0];
  return <><Link className="back" href="/assets">← Assets</Link><Header title={asset.hostname} eyebrow="Hostname"><p>{latest ? `Last observed ${date(latest.snapshot.finished_at)}` : "Not yet observed"}</p></Header><RunControl asset={asset} latest={latest} busy={busy} onRun={onRun} /><RecommendedChecks run={latest} />{latest ? <><Changes current={latest} previous={previousRun(workspace, latest)} /><section className="section"><div className="section-heading"><h2>Current observations</h2><Link href={`/runs/${latest.id}`}>Open latest results →</Link></div><Counts run={latest} /><ObservationList run={latest} /></section></> : null}<section className="section"><h2>Run history</h2><p className="quiet">Previous source snapshots remain intact when you run again.</p><History workspace={workspace} assetId={asset.id} /></section></>;
}

function downloadSource(run: Run) {
  const url = URL.createObjectURL(new Blob([JSON.stringify(run.snapshot)], { type: "application/json" }));
...
function Missing({ title = "Not found" }: { title?: string }) {
  return <div className="empty"><h1>{title}</h1><p>This item is not in the current local workspace.</p><Link className="button secondary" href="/">Back to overview</Link></div>;
}
...
export function ProductApp() {
  ...
  useEffect(() => {
    ...
    fetch("/api/workspace", { credentials: "same-origin", cache: "no-store" }).then(async (response) => {
      ...
      if (!response.ok) throw new Error("The local workspace could not be loaded.");
      return response.json() as Promise<Workspace>;
    }).then((value) => { if (active) setWorkspace(value); }).catch((cause: unknown) => { if (active) setError(cause instanceof Error ? cause.message : "The workspace could not be loaded."); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);
  ...
  async function refresh() {
    setWorkspace(await request<Workspace>("/api/workspace"));
  }
  async function onAdd(hostname: string) {
    await perform(async () => {
      const asset = await request<Asset>("/api/assets", "POST", { hostname });
      await refresh();
  ...
  async function onRun(asset: Asset) {
    await perform(async () => {
      const run = await request<Run>("/api/runs", "POST", { assetId: asset.id, authorized: true });
      await refresh();
  ...
  if (!workspace) {
    content = <div className="welcome"><p className="eyebrow">WitnessOps · External Exposure</p><h1>Understand what the outside world can observe.</h1><p>Add a hostname, run recommended checks, inspect the evidence, and see what changed when you return.</p><button className="button" disabled={busy || loading} onClick={() => void perform(async () => { setWorkspace(await request<Workspace>("/api/session", "POST", {})); router.push("/"); })}>{loading ? "Loading workspace…" : busy ? "Opening…" : "Start local workspace"}</button><div className="foundation-note"><strong>Local foundation</strong><p>This is a development session, not customer authentication. Assets and snapshots expire with the local session. Do not use private customer data.</p></div></div>;
  } else if (!parts.length) {
    content = <Overview workspace={workspace} />;
  } else if (parts[0] === "assets" && parts.length === 1) {
    content = <><Header title="Assets" action={<Link className="button" href="/assets/new">Add asset</Link>}><p>Add what you want WitnessOps to observe. Adding an asset does not prove ownership.</p></Header><AssetList workspace={workspace} /></>;
  } else if (pathname === "/assets/new") {
    content = <AddAsset busy={busy} onAdd={onAdd} />;
    ...
    content = <><Header title="Members"><p>Access is explicit. An email domain or workspace name does not establish membership.</p></Header><div className="member-row"><span className="avatar" aria-hidden="true">L</span><div><strong>Local developer</strong><p>Development session · not a verified customer identity</p></div><span className="status">Local only</span></div><p className="boundary">Invitations and customer membership are not connected in this foundation. No invitation will be sent.</p></>;
  } else if (pathname === "/settings") {
    content = <><Header title="Settings"><p>The workspace holds this local session’s assets and observations.</p></Header><dl className="facts settings-facts"><div><dt>Workspace</dt><dd>{workspace.name}</dd></div><div><dt>Session expires</dt><dd>{date(workspace.expiresAt)}</dd></div><div><dt>Authentication</dt><dd>Development seam only. Customer authentication is not connected.</dd></div><div><dt>Persistence</dt><dd>Temporary server session. No production customer database or cross-device access.</dd></div><div><dt>Collection</dt><dd>Manual, explicitly authorized hostname observations. No schedules.</dd></div></dl><RecommendedChecks /><div className="clear-session"><h2>Clear local session</h2><p>This removes this session’s assets and runs. Download any source snapshots you need first.</p><button className="button secondary" disabled={busy} onClick={() => void clearSession()}>Clear local session</button></div></>;
  } else {
    content = <Missing />;
  }
  return <div className="product-app"><a className="skip-link" href="#main-content">Skip to content</a><aside className="desktop-sidebar"><Link className="wordmark" href="/" aria-label="WitnessOps overview"><span aria-hidden="true">◉</span> WitnessOps</Link>{navigation}<p className="sidebar-footer">External Exposure<br /><span>Local foundation</span></p></aside><div className="app-body"><header className="app-header"><button className="menu-toggle" aria-expanded={navOpen} aria-controls="mobile-navigation" onClick={() => setNavOpen(!navOpen)} aria-label={navOpen ? "Close navigation" : "Open navigation"}>☰</button><span className="workspace-name">{workspace?.name || "WitnessOps"}</span><span className="dev-label">Local foundation</span><span className="account">Local developer</span></header>{navOpen ? <div className="mobile-navigation" id="mobile-navigation">{navigation}</div> : null}<main id="main-content" className="main-content">{error ? <div className="error" role="alert">{error}<button onClick={() => setError("")} aria-label="Dismiss error">×</button></div> : null}{busy && workspace ? <p className="pending" role="status">Completing your request…</p> : null}{content}</main></div></div>;
}
