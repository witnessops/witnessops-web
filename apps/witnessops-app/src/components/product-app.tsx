...
function AssetPage({ workspace, asset, busy, onRun }: { workspace: Workspace; asset: Asset; busy: boolean; onRun: (asset: Asset) => Promise<void> }) {
  const latest = orderedRuns(workspace, asset.id)[0];
  return <><Link className="back" href="/assets">← Assets</Link><Header title={asset.hostname} eyebrow="Hostname" action={workspace.role === "owner" ? <a className="button secondary" href="#run-observation">{latest ? "Run again ↓" : "Run observation ↓"}</a> : undefined}><p>{latest ? `Last observed ${date(latest.snapshot.finished_at)}` : "Not yet observed · adding this hostname did not run any checks."}</p></Header>{latest ? <><ResultSummary run={latest} /><Changes workspace={workspace} current={latest} previous={previousRun(workspace, latest)} /></> : <RecommendedChecks expanded />}{workspace.role === "owner" ? <RunControl asset={asset} latest={latest} busy={busy} onRun={onRun} /> : <p className="quiet boundary">Viewer access · Only an Owner can start an observation.</p>}{latest ? <><RecommendedChecks /><section className="section" id="all-observations"><h2>All observations</h2><ObservationList run={latest} /></section></> : null}<section className="section"><h2>Run history</h2><p className="quiet">Previous evidence stays intact. Rerun later to compare; no monitoring happens automatically.</p><History workspace={workspace} assetId={asset.id} /></section></>;
}
