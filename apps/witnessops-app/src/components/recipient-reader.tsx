...
export function RecipientReader({ model, digest, publishedAt, expiresAt, passwordProtected = false, signupUrl='/signup', preview = false }: {
    ...
    return <div ref={root} className={styles.reader}>
        ...
        {!preview && <div className={styles.print}><header><h1>{title}</h1><p>Publisher-supplied report name · Fixed recipient revision</p><p>Snapshot SHA-256: {digest}</p>{publishedAt && <p>Published {date(publishedAt)}</p>}<p>Source evidence is not included in this share.</p></header><BuyerReportDocument model={model} recipientPrint/></div>}
    </div>;
}
