...
export function AccessGate({ state, busy, activate }: { state: EarlyAccessState; busy: boolean; activate: () => void }) {
  return <div className="welcome"><p className="eyebrow">WitnessOps checks · Early Access</p><h1>{state === 'invited' ? 'Your workspace access is ready' : state === 'paused' ? 'Workspace access is paused' : 'Request workspace access'}</h1><p>{state === 'invited' ? 'Open a workspace and choose a public hostname check or an operator-assisted Linux server check.' : state === 'paused' ? 'Your access is paused. Saved workspace data has not been deleted. Contact us to discuss access.' : 'Workspace access is currently by invitation. Signing in does not automatically grant access. You can use the free hostname check without an account.'}</p><p className="quiet">A public /check snapshot is not imported. You will explicitly authorize a fresh saved observation in your workspace.</p><div className="actions">{state === 'invited' ? <button className="button" disabled={busy} onClick={activate}>Activate workspace access</button> : <a className="button" href={requestAccess}>{state === 'paused' ? 'Contact WitnessOps' : 'Request workspace access'}</a>}<a className="button secondary" href="https://witnessops.com/check">Run a free check</a><form action={logout}><button className="button secondary">Sign out</button></form></div><p className="quiet">{EARLY_ACCESS_DATA_NOTE}</p></div>;
}
...
export function DeeperReview({ run }: { run: Run }) {
  const { event } = useProductActivity();
  return <aside className="section deeper-review" aria-label="Paid review"><h2>Need help deciding what to fix?</h2><p>{EXTERNAL_ATTACK_SURFACE_OFFER.name.en} · {EXTERNAL_ATTACK_SURFACE_OFFER.price.en}</p><p className="quiet">A separate, human-reviewed investigation of one authorized public-facing system, with findings, evidence and a buyer report. We agree fit and scope before work begins.</p><a className="text-action" href={`https://witnessops.com${EXTERNAL_ATTACK_SURFACE_OFFER.route.en}`} onClick={() => event('deeper_review_clicked', run)}>See the External Attack Surface Review scope →</a></aside>;
}
