...
import { printBuyerReport } from '@/lib/proofpack/report-model';
...
export function ProofpackWorkspace() {
    ...
    const exportPdf = () => { printBuyerReport(model, () => window.print()); };

    return <main id="main-content" className={styles.workspace}>
 <header className={styles.header}><a href="/" className={styles.brand}>WITNESSOPS<span> / Proofpack Report</span></a><span className={styles.local}>LOCAL AUDIT 1.2.2</span></header>
 <div className={`${styles.screen} ${tab === 'Report' && report ? styles.reportOpen : ''}`}>
 {!report && <section className={styles.intro}><span className={styles.eyebrow}>EVIDENCE YOU CAN READ</span><h1>Open your proofpack.</h1><p>See what was checked, what needs attention and what remains unknown.</p><p className={styles.privacy}>Selected files are processed on this device. They are not uploaded to WitnessOps.</p></section>}
 <form onSubmit={verify} ref={formRef} className={`${styles.inputPanel} ${report ? styles.compact : ''}`} aria-label="Open proofpack">
 <div className={styles.bundleIntake}><div className={styles.file}><strong id="bundle-label">Proofpack file</strong><span id="bundle-help">Local Audit 1.2.2 · .proofpack · maximum 110 MiB</span><input ref={fileRef} className={styles.fileInput} type="file" aria-labelledby="bundle-label" aria-describedby="bundle-help" accept=".proofpack" onChange={e => selectFile(e.target.files?.[0] ?? null)} /><button type="button" onClick={() => fileRef.current?.click()}>Choose Proofpack</button><span className={styles.selectedFile} aria-live="polite">{file?.name ?? 'Choose a file to enable verification.'}</span></div>
 <div className={styles.trustSource}><span>Trust source</span><strong>{LOCAL_AUDIT_TRUST.name} · v{LOCAL_AUDIT_TRUST.version}</strong><code>sha256:{LOCAL_AUDIT_TRUST.sha256}</code><small>Supplied independently by this verifier.</small></div></div>
 <details className={styles.bundleDetails}><summary>Verification details</summary><p>The bundle contains a Local Audit package and its detached signature. The signature covers the enclosed package, not the outer envelope. Both are untrusted until verification succeeds.</p><p>The production registry is pinned in this application. A bundle cannot supply or replace it.</p><p>Selected files are processed on this device. They are not uploaded to WitnessOps.</p><p>Bundle format: witnessops-proofpack-bundle, version 1.</p></details>
 <div className={styles.actions}><button className={styles.primary} disabled={busy || !file}>{busy ? 'Checking package locally…' : 'Verify locally'}</button><button type="button" onClick={clear}>Clear</button>{busy && <button type="button" onClick={() => { abortRef.current?.abort(); }}>Cancel verification</button>}</div>
 </form>
 ...
 <p className={styles.boundary}>These checks establish the package and its declared bindings. They do not establish that the server is secure, uncompromised or compliant.</p></section>
