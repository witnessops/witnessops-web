...
import type { WizOperatorState } from "./wiz-operator-mark";
...
export function buildAdminWizBrief(
  ...
  if (input.divergent > 0) {
    return {
      ...
      detail:
        "Wiz will not infer a resolution from conflicting evidence. Review the divergence before advancing the work.",
  ...
  if (input.reconciliationPending > 0) {
    return {
      ...
      detail:
        "Unresolved evidence is waiting for an operator decision. Wiz recommends checking the pending queue next.",
  ...
  if (input.ready > 0) {
    return {
      ...
      detail:
        "The current admission summary shows work ready for review. Wiz recommends starting there.",
  ...
  if (input.total === 0) {
    return {
      ...
      detail:
        "No intakes are present in the current admission summary. Nothing is being executed automatically.",
  ...
  return {
    ...
    detail:
      "The current admission summary does not show divergent, pending, or ready work requiring an immediate decision.",
    actionLabel: "Open queue",
    actionHref: "/admin/queue",
  };
}
