...
export function generateMetadata(): Metadata {
  const doc = loadLegalPage("terms");

  return {
    title: doc?.title ?? "Terms of Service",
    description: doc?.description ?? "WITNESSOPS terms of service.",
    alternates: getCanonicalAlternates("witnessops", "/terms"),
  };
}

export default function TermsPage() {
  const doc = loadLegalPage("terms");
  const relatedDocs = loadLegalIndex().filter((entry) => entry.slug !== "terms");
