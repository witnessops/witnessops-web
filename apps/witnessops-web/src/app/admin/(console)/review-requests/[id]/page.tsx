...
import { AdminReviewBriefForm } from "@/components/admin/admin-review-brief-form";
import { hasBusinessAuthority } from "@/lib/server/admin-authorization";
import { reviewRequestEditVersion } from "@/lib/server/admin-core-spine";
...
import { getCustomer, getReviewRequest, listAuditEvents, listProductContractChoicesForReview } from "@/lib/server/admin-core-spine";
...
export default async function AdminReviewRequestDetailPage({ params }: RouteContext) {
  ...
  if (!request) notFound();
  const [customer, products, events] = await Promise.all([getCustomer(request.customerId, actor), listProductContractChoicesForReview(request.id, actor), listAuditEvents(request.lineageId, actor)]);
  const editable = hasBusinessAuthority(actor.role ?? "Founder") && !request.productContractVersionId && !request.proofRunId && !["declined", "closed"].includes(request.state);
  return <CorePage title={customer ? `Review for ${customer.name}` : "Review request"} eyebrow={request.id}>
    ...
    {editable ? <details className={styles.coreDetailSection}><summary className={styles.briefSummary}>Edit qualification and next action</summary><AdminReviewBriefForm requestId={request.id} initial={{ scope: request.scope, desiredOutcome: request.desiredOutcome, timing: request.timing, commercialStatus: request.commercialStatus, nextAction: request.nextAction, workflowBoundary: request.workflowBoundary, authorityBoundary: request.authorityBoundary, evidencePosture: request.evidencePosture, missingInformation: request.missingInformation }} version={reviewRequestEditVersion(request)} /></details> : <p className={styles.coreFormNote}>Qualification is read-only for this request or your current role.</p>}
