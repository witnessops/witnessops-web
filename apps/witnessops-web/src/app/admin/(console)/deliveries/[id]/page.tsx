...
import { CoreAction, DeliveryDraftForm, DeliverySendReconciliationForm, ReceiptLinkForm } from "../../../../../components/admin/admin-core-ui";
import { CoreAuditTimeline, CoreCard, CoreMeta, CorePage, CoreState } from "../../../../../components/admin/admin-core-view";
import { buildDeliveryReadiness, getDelivery, getDeliverySendReservationStatus, listAuditEvents } from "@/lib/server/admin-core-spine";
...
export default async function AdminDeliveryDetailPage({ params }: RouteContext) {
  ...
  const [readiness, events, sendReservation] = await Promise.all([buildDeliveryReadiness(id, actor), listAuditEvents(delivery.lineageId, actor), getDeliverySendReservationStatus(id, actor)]);
  return <CorePage title={delivery.id} eyebrow="Delivery">
    ...
    {sendReservation?.status === "reserved" || sendReservation?.status === "outcome_unknown" ? <CoreCard title="Unresolved send reservation"><p className={styles.coreText}>The provider outcome must be confirmed before this delivery can be sent again or advanced.</p><DeliverySendReconciliationForm deliveryId={delivery.id} /></CoreCard> : null}
