...
export default async function AdminInboxDetailPage({ params }: RouteContext) {
  ...
  return <CorePage title={item.subject} eyebrow="Inbox item">
    ...
    <div className={styles.coreDetailSection}><div className={styles.coreDetailSectionTitle}>Lifecycle action</div>{item.state === "security-routed" ? <p className={styles.coreText}>This message is routed to {"security@witnessops.com"} and cannot become a review request.</p> : item.reviewRequestId ? <p className={styles.coreText}>Linked review request: <Link href={`/admin/review-requests/${item.reviewRequestId}`} className={styles.inlineLink}>{item.reviewRequestId}</Link></p> : actor.role === "Founder" ? <CoreAction endpoint={`/api/admin/core/inbox/${item.id}/convert`} body={{ idempotencyKey: `convert:${item.id}` }} label="Convert to review request" confirmText="Create an internal review request from this Gmail item?" /> : <p className={styles.coreText}>A Founder must create the initial review request.</p>}</div>
