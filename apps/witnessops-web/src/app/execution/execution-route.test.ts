...
test("execution index route redirects to proof-backed security systems", () => {
  const source = readFileSync(resolve(__dirname, "page.tsx"), "utf-8");

  assert.match(
    source,
    /permanentRedirect\(\"\/proof-backed-security-systems\"\)/,
    "Execution index route should redirect to the canonical proof-backed security systems page.",
  );
  assert.doesNotMatch(
    source,
