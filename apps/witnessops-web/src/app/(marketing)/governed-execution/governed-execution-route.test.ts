...
test("governed-execution route redirects to proof-backed security systems", () => {
  const source = readFileSync(resolve(__dirname, "page.tsx"), "utf-8");

  assert.match(
    source,
    /permanentRedirect\(\"\/proof-backed-security-systems\"\)/,
    "Governed execution route should redirect to the canonical proof-backed security systems page.",
  );
  assert.doesNotMatch(
    source,
