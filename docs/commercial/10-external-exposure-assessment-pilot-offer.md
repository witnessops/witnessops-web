...
- Passive discovery and pre-approved low-impact DNS, TLS, HTTP(S), service-identification, and exposure checks.
- Unauthenticated, outside-in review only.
- Manual validation, deduplication, prioritisation, and remediation guidance.

The signed target schedule controls the actual scope. The caps are maximums, not permission to test every discovered asset.

### What you receive

- Authority, target, approved-check, exclusion, and stop-condition record.
- External exposure map for the confirmed scope.
- Prioritised findings tied to evidence references.
- Practical remediation recommendations.
- Buyer-readable executive report and technical appendix.
- Evidence manifest and artifact hashes.
- Explicit unknowns, limitations, and recommended deeper work.
- Signed receipt and offline verification package where the supported path is successfully produced. This verifies the delivered package mechanism, not that the assessed system is secure.
...
Send a non-secret fit check describing the decision, deadline, intended receiving party, high-level target boundary, authority status, and whether you need a vulnerability assessment or an exploitation-led pentest. Do not send credentials, logs, screenshots, source exports, customer records, private IP inventories, or production evidence before handling and scope are agreed.

Use: <https://witnessops.com/review/request>
