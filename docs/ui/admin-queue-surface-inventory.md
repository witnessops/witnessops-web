...
| /api/contact | Direct review request email | POST /api/contact | phase1-authoritative | n/a | sendMail (file/resend/m365) | approved | Email-only path to engage@mail.witnessops.com; does not write queue state |
| /api/support/message | Direct support request email | POST /api/support/message | phase1-authoritative | n/a | sendMail (file/resend/m365) | approved | Email-only path to engage@mail.witnessops.com; does not write queue state |
...
| /admin/login | Google Workspace sign-in | GET /api/admin/google/start | phase1-authoritative | n/a | Google-bound admin session issuance | approved | POST callback at /api/admin/google/callback |
| /api/admin/google/callback | Google Workspace OIDC callback | POST /api/admin/google/callback | phase1-authoritative | n/a | Google-bound admin session cookie | approved | Size-bounded form-post; no authorization code in URL |
