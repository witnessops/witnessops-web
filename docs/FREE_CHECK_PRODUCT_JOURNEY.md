...
| Open a workspace | Configured app link from the result; `/early-access` explains access and the journey | The existing route is retained. With approved self-service enabled, verified accounts create their own workspace. Joining another workspace requires an Owner invitation; login alone grants no membership. |
| Save a baseline | Verify email and create/open your own workspace, or accept an Owner invitation, add the hostname, explicitly authorize a new observation | Adding a hostname does not collect. The saved run is a new observation, not the earlier public snapshot. |
