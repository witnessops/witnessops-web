...
[[ "$(docker image inspect "${config}" --format '{{.Os}}/{{.Architecture}}')" == linux/amd64 ]]
[[ "$(docker image inspect "${config}" --format '{{.Config.User}}')" == nextjs ]]
docker run --rm --network none --entrypoint /bin/sh "${config}" -eu -c '
  test "$(id -u)" = 1001
  ...
  gws --version
'
container="$(docker run -d -p 127.0.0.1::3000 "${config}")"
...
[[ "$(docker image inspect "${image}" --format '{{.Id}}')" == "${config}" ]]
