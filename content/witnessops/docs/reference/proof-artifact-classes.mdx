...
| `verification result` | Verification contract + verify response contract | `live output term` | [How to Verify a Receipt](/docs/how-it-works/verification), [Receipt Spec](/docs/evidence/receipt-spec), `/api/verify` responses | Output semantics are `valid`, `invalid`, `indeterminate` at contract level (details below). |
