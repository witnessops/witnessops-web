...
## Fleet and mesh operators (OffSec lane)

Public buyer docs describe the proof package path. **Fleet operators** on the
WitnessOps mesh use a different entry surface:

| Need | Where |
| --- | --- |
| Mesh operator docs (synced bundle) | [hacktheworld.zip/mesh-docs/](https://hacktheworld.zip/mesh-docs/) |
| Hunt / verify CLI | `wops hunt route`, `wops hunt loop`, `wops hunt verify` (after `vm-tooling-env.sh`) |
| OffSec lane sync + deploy | OffSec-Lane `sync-to-node.sh`, `docs/local-mesh/COMMAND-CATALOG.md` |

The **runbook helper commands below** apply to the `witnessops-offsec` engagement
layout (`ops/engagements/`). They are not the full fleet hunt loop.
