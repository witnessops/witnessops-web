---
title: "Commands"
description: "Current app CLI authentication commands, followed by separate operator runbook reference."
section: "docs"
order: 10
nav_label: "Commands"
draft: false
...
Start with [CLI setup and authentication](/docs/getting-started/cli). The package is private source, not a published npm installer. From the approved repository checkout with Node.js 22:
...
Login authorizes a CLI session, not collection or signing. Use [Account and CLI help](/docs/getting-started/access-help) for expired sessions, role restrictions or access problems.

## Separate operator runbook reference

The remainder describes the `witnessops-offsec` engagement toolchain, not the app CLI. Its helper scripts are not supplied by this web repository. Use it only with the separately supplied toolchain and its matching contract. These commands do not install the app or grant workspace access.
...
Public buyer docs describe the proof-package path only. **Fleet / mesh operators**
use internal OffSec mesh documentation and tooling that is **not published on this
docs site** and is not part of the public product contract.

The **runbook helper commands below** apply to the `witnessops-offsec` engagement
layout (`ops/engagements/`) when that layout is in use. They are not a public
hunt loop and do not replace the buyer proof path on `/verify`.
