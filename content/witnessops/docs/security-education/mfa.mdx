...
Teams often deploy MFA and then trust it too much. This page covers the operational gap: attackers still gain access through user pressure, real-time relay, token reuse, and weak fallback paths.
...
**Attacker path:** attacker targets weaker recovery or fallback methods such as SMS, voice, helpdesk override, recovery codes, or email reset paths.

**Operational point:** the strongest configured factor does not help if weaker fallback routes remain exploitable.

## Observable signals and what each can support

| Signal | What it can support | What it cannot prove alone |
|---|---|---|
| Burst of denied or unanswered MFA prompts | Active use of valid credentials against an account | User compromise or attacker identity |
| Successful sign-in following prompt burst | Increased likelihood of fatigue approval or related misuse | Exact mechanism, such as fatigue vs relay vs insider |
| New session from unusual ASN or device after phishing lure | Plausible relay or session-theft hypothesis | Definitive attribution to one campaign |
| Session activity without a nearby fresh MFA challenge | Token or session reuse hypothesis | Initial theft method |
...
| Helpdesk or manual bypass event | Fallback route was exercised | Whether bypass was malicious or approved |

## Response decisions and escalation gates

Use governed decisions tied to evidence strength:

1. **Contain the account immediately** — revoke sessions, force credential reset, and require re-auth when suspicious access is observed, not only when intent is confirmed.
...
3. **Escalate to incident response** when any of these gates are met: privileged account impact, confirmed post-auth session misuse, multiple users in one campaign, or sensitive-system access.
...
- signed decision records and action receipts for containment and escalation steps
...
- **Observed:** raw auth events, prompt counts, timestamps, source network or device metadata, factor-change events, executed containment actions.
...
- location and device risk signals are probabilistic
...
MFA abuse commonly begins with valid credentials. Continue to [Password Reuse](/docs/security-education/password-reuse) for the upstream exposure path that often seeds these attacks.
