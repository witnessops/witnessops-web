...
PURPOSE
     ...
     Use this page only as a supplemental draft operator reference for
     command semantics, lifecycle order, and artifact expectations.
     It is not a frozen public product contract.
...
COMMAND USAGE SEMANTICS
     new-engagement
         Initializes ops/engagements/<engagement-name>/ and creates the workspace structure.
     ...
     runbook-exec
         ...
         and writes terminal artifacts when the run completes.
         Terminal runs emit state.json, manifest.json, hash-manifest.txt,
         runbook-summary.md, and receipt.json.

     runbook-state
         Read-only run inspection.
         Reports status and step progress.
         Does not mutate run state, approval state, or step flow.
...
NEXT PAGE
     Continue to /docs/reference/commands for the fully indexed command contract.
