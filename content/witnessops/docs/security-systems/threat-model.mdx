...
description: "Operator reference: What WitnessOps can prove, what it assumes, and what to do when a proof claim is challenged."
section: "docs"
order: 40
...
Not every incomplete verification result is a hard failure. The verifier can return `indeterminate` when an input is structurally coherent but required evidence bytes, request or workflow records, scope authorization, signature verification, signer authorization, or another trust dependency was not independently checked.

That distinction matters. A check that was not possible is different from a check that ran and proved a mismatch.
...
A partial package may contain individual checks that pass, but it cannot produce a production-valid `public_exposure_review` profile result while required package or trust inputs are missing.

A verifier may be able to confirm that a presented subset is internally consistent while the overall result remains incomplete or `indeterminate`. The subset does not establish that it represents the whole engagement, chain, or decision path.
...
- `valid` means the declared checks for the selected verification mode passed
- `invalid` means a proof-bearing check failed
- `indeterminate` means the input may be coherent, but one or more required evidence, workflow, authorization, signature, or trust checks were not independently completed

On the public receipt-only `/verify` surface, an internal `limited-pass` maps to `indeterminate`. The legacy-named `public_exposure_review` profile also remains `indeterminate` until the full evidence and production trust checks are independently completed.
