...
A partial package may contain individual checks that pass, but it cannot produce a production-valid Public Exposure Review result while required package or trust inputs are missing.
...
On the public receipt-only `/verify` surface, an internal `limited-pass` maps to `indeterminate`. Public Exposure Review also remains `indeterminate` until the full evidence and production trust checks are independently completed.
