...
If you are evaluating the public request path before submitting work, read the [Buyer path](/docs/getting-started/proof-run-buyer-path) for catalogue, sample, verifier and non-secret fit-check order.
...
Use [Verify a receipt](/verify) to check receipt JSON on the public tool. For procedure detail, read [How to Verify a Receipt](/docs/how-it-works/verification) and [Receipt Specification](/docs/evidence/receipt-spec).
