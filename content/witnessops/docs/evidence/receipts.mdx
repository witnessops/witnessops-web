...
App outputs differ: External Exposure snapshots are unsigned, reports are derived presentations, and Local Audit Proofpacks have a separate signed-package checking path. Start with [Understand results and reports](/docs/getting-started/results). This page focuses on receipt profiles, not every app result.
