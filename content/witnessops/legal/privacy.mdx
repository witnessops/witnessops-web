...
See [Cloudflare Web Analytics](https://developers.cloudflare.com/web-analytics/about/) for the provider's measurement approach.
...
### Authenticated workspaces, including Early Access
...
Supported Linux imports also retain the uploaded source files and their verification context. Report sharing creates a separate, fixed copy of selected report content. Revoking a share prevents subsequent access through that link; it does not recall content already seen, downloaded or delivered to a recipient.

### Requesting removal

Use [Support](/support#support-request) to request removal. Identify the account or workspace concerned and whether you want saved results removed, shared links revoked, or the account closed. Do not attach source evidence, passwords, credentials or identity documents to the initial request. We may need to confirm your identity and authority over a shared workspace before acting.

Removal needs a stated scope. Deleting saved workspace content is different from closing an account, deleting identity-provider records, or removing copies in backups, logs, correspondence and external providers. Historical agreement records and reserved reporting names may remain. A completion response must identify what was removed, what remains, and any unresolved copies or retention requirements; a request acknowledgement or link revocation alone is not confirmation of deletion.

Until the relevant removal scope and retention arrangements have been confirmed, use non-sensitive evaluation data. A stated retention period is not evidence that automatic expiry is operating.
