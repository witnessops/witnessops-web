...
try {
 ...
 assert.equal((await db.query('SELECT count(*) AS n FROM app_migrations')).rows[0].n,'13');
 ...
 await migrate(db);
 console.log('PASS: populated 0004 -> 0013, historical run/source/digest unchanged, no implicit plan/consent/usage, immutable trigger retained, reapply idempotent');
