...
export async function serverCheck(args,options={}){
 if(args.length!==2||args[0]!=='server'||args[1]!=='check')throw new Error('Use sudo wops server check. No remote, runtime, profile or signer overrides are supported.');
 ...
 return io.lock(base,async()=>{
  ...
  if(!state){
   ...
   try{const purpose=await ask('Reason for this authorized check: '),sshExposure=await ask('Intended SSH exposure (public/private/none): '),expectedListeners=listeners(await ask('Intended listener endpoints (tcp://IP:port, udp://[IPv6]:port), or none: '));
    ...
    if((await ask('I control this server and authorize one read-only check now (30-minute window), including capture upload. Proceed? [y/N] ')).toLowerCase()!=='y'){output('Cancelled. No collection.');return 0;}
    state={request:{requestId:randomUUID(),assetId:candidates[0]?.id??null,hostname:host,purpose,sshExposure,expectedListeners}};await io.privateWrite(journal,JSON.stringify(state));
   }finally{rl?.close();}
  }
